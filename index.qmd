...
bluffbench_diamonds$carat <-
  max(bluffbench_diamonds$carat) - bluffbench_diamonds$carat

bluffbench_diamonds_plot <-
  ggplot(bluffbench_diamonds, aes(x = carat, y = price))

ggsave(
  "figures/bluffbench-diamonds-carat-price-thumb.png",
  bluffbench_diamonds_plot + geom_point(size = 2, alpha = 0.3),
  width = 3,
  height = 2.5
)
...
Please plot price vs carat in diamonds
:::
...
![](figures/bluffbench-diamonds-carat-price-thumb.png){fig-alt="A small scatterplot thumbnail with carat on the x-axis and price on the y-axis. The points form a strong negative association." width="260px" style="border-radius: 12px; display: block; margin: 10px auto 0 auto;"}
...
<div style="margin: 18px 0 0; font-size: 44px; font-weight: 700; color: #3f73d8;">Can models interpret plots that contradict their expectations?</div>
...
continuing the analysis. a careful data analyst would probably spot it and investigate.
:::
...
::: {.incremental}
1. The models got better
2. But that doesn't solve everything!
3. We want to continue making new software to explore data
:::
...
:::notes
Sara described her first moments of really getting the hang of data science tools as like seeing below the surface. I think many of us might have had a moment like this at some point, when we began to understand how powerful data science and statistics are for answering questions about the world.
...
:::notes
Let's revisit those lessons learned on agent-building from Sara. She said we ought to help the model not be wrong, and make it less bad when it is wrong.
...
You review the agent's behavior, adding trusted code to move responses up the trust ladder.
:::
:::
...
## {.agent-examples-slide .paper-slide}
...
<table>
  ...
  <tbody>
    <tr class="fragment fade-in">
      ...
      <td>Biologist / MD / clinical researcher</td>
      <td>&ldquo;Compare treatment groups using the approved method.&rdquo;</td>
    </tr>
    <tr class="fragment fade-in">
      <td>UX researcher</td>
    ...
    <tr class="fragment fade-in">
      <td>Economist / policy analyst</td>
      <td>Policy maker / program lead</td>
...
<video class="rounded click-to-toggle" width="100%" preload="auto" tabindex="0" aria-label="Clinical Trials Agent demo. Click to play or pause.">
  <source src="clinical-trials-agent-full-demo.mp4" type="video/mp4">
</video>
