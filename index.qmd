...
## The models got better {.draft-slide}

## {.draft-slide .paper-slide}
...
![](figures/bluffbench-tips-thumb.png){fig-alt="A small scatterplot thumbnail with total_bill on the x-axis and tip on the y-axis. The points drift downward from upper left to lower right, suggesting a negative association." width="160px" style="border-radius: 12px; display: block;"}
:::
...
The model receives the rendered plot. Here, the relationship is negative, but
the model describes the familiar positive relationship from the original data.
:::

## {.draft-slide .paper-slide}

![](figures/bluffbench-tips-plot.png){fig-alt="A full-size scatterplot of total bill against tip. The points form a clear negative association, with tips generally falling as total bill rises." width="92%"}

## {.draft-slide .paper-slide}

![](figures/original-bluffbench-results.png){fig-alt="A horizontal stacked bar chart titled 'Models often report what they expect to see, not what's plotted.' GPT-5, Gemini Pro 2.5, and Claude Sonnet 4.5 are almost entirely incorrect." height="560"}
...
## The models got better {.draft-slide .paper-slide}

![](figures/latest-bluffbench-results.png){fig-alt="A faceted horizontal stacked bar chart of percent correct across recent models, split into thinking and no-thinking groups. Thinking models perform better, while most models still give many incorrect answers." height="560"}
...
## bluffbench2 {.draft-slide .paper-slide}

![](figures/bluffbench2-bmi-cholesterol.png){fig-alt="A scatterplot of BMI against cholesterol with a positive trend. A subset of points falls exactly along a straight line, suggesting a data-quality artifact." width="92%"}
...
In this example, most points follow a noisy positive relationship, but a subset
falls exactly on a straight line. This pattern can indicate deterministic
imputation that should be investigated before continuing the analysis.
:::

## bluffbench2 {.draft-slide .paper-slide}

![](figures/bluffbench2-results.png){fig-alt="A horizontal bar chart of BluffBench2 scores by model. Every model scores below 20 percent, showing that frontier models still struggle to notice subtle data-quality issues." width="92%"}
...
Posit makes software to help you understand data.
...
::: {.statement}
Our implicit strategy: [read the code.]{.accent}
:::
...
# People are not fixed safety components {.paper-slide}
...
<br>
<br>
* **Approval fatigue:** people accepted 93% of permission requests
<br>
<br>
* **Context:** trust, social cues, interaction style, cognitive load
:::
--> 
...
The question is not [who checks the output?]{.accent}
...
::: notes
In Posit Assistant, the user is typically a data or code minded person. The interface assumes they can inspect the work.
:::

# The VP's analysis was wrong
...
::: notes
The VP had good intentions. She wanted an answer to her question, and reached for a tool convenient to her. 

unfortunately, the answer she got was wrong.

the next day, a data scientist checked her analysis, noticed the mistake. not only do they notice the problem, they already have a couple functions in an internal package that would have carried out the analysis correctly for the vp.
:::

# [_Trusted_ code]{style="color: #ee6331; font-size: 1.4em;"}

::: notes
this is _trusted_ code
...
We can leverage this code instead of having the model recreate the analysis every time.

this is one of the core ideas behind the new open source package commons
:::
...
::: {.incremental}
* Open source in R and Python

* Built on `ellmer` / `chatlas` and `shinychat`

* For data scientists to build for everyone else
:::
...
# Start with trusted calculations

```{=html}
<video class="rounded click-to-toggle" width="66%" preload="auto" tabindex="0" aria-label="Commons trusted calculation demo. Click to play or pause.">
  <source src="figures/vp-agent-trusted.mp4" type="video/mp4">
</video>
```
...
# Allow custom code when needed

```{=html}
<video class="rounded click-to-toggle" width="66%" preload="auto" tabindex="0" aria-label="Commons custom query demo. Click to play or pause.">
  <source src="figures/vp-agent-custom-query.mp4" type="video/mp4">
</video>
```
...
# Context supports the custom path

```{=html}
<video class="rounded click-to-toggle" width="66%" preload="auto" tabindex="0" aria-label="Commons context-supported answer demo. Click to play or pause.">
  <source src="figures/vp-agent-context.mov" type="video/mov">
</video>
```
...
# Deterministically tag trust

![](figures/commons-trust-flow.png){width=85%}
...
* Help the model not be wrong **with trusted calculations and context**
* Make it less bad when it is wrong **with provenance and feedback loops**

# Review logs for continual improvement

```{=html}
<video class="rounded click-to-toggle" width="66%" preload="auto" tabindex="0" aria-label="Commons trajectory review demo. Click to play or pause.">
  <source src="figures/vp-agent-trajectory-review.mp4" type="video/mp4">
</video>
```

::: notes
review for correctness, add trusted code for commonly asked questions
improve context

Recurring
questions can become trusted calculations. problems with table choice, joins,
definitions, or caveats can become better context.
:::
...
## commons works with tools you already use
